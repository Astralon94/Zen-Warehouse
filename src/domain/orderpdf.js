...
export function orderDocHtml(locale, order, dp) {
  ...
  // Note per fornitore: snapshot preso all'invio (fedeltà nella ristampa dallo Storico).
  ...
  return groups.map((g, i) => {
    ...
    return `<div ${i > 0 ? 'style="page-break-before:always"' : ''}>
      ...
      ${supNote ? `<div class="notebox"><b>📝 Nota:</b> ${esc(supNote)}</div>` : ''}
