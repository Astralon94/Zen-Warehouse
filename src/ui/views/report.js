...
// Metrica PRINCIPALE = la SPESA in € (qty × prezzo di acquisto per riga); i pezzi restano come
// dato secondario. Filtri combinabili (periodo / intervallo date / fornitore / categoria) +
...
// euro compatto per etichette fitte dei grafici (assi, cime barre, centro ciambella)
const eurC = n => { n = Math.round(+n || 0); return n >= 1000 ? '€' + (n / 1000).toFixed(n >= 10000 ? 0 : 1) + 'k' : '€' + n; };
...
// ---- grafico a barre verticali SVG (andamento mensile della SPESA) ----
// months: [{month,spend,pieces,orders}]. Asse X = mesi, Y = spesa €; valore € sopra ogni barra.
function barChart(months) {
  const W = Math.max(320, months.length * 64 + 48), H = 220;
  const padL = 44, padR = 12, padT = 22, padB = 34;
  ...
  yTicks.forEach(v => {
    ...
    g += `<text x="${padL - 6}" y="${y + 3}" text-anchor="end" font-size="9" fill="var(--sub)">${eurC(v)}</text>`;
  });
  months.forEach((m, i) => {
    const cx = padL + bw * i + bw / 2;
    const bh = (m.spend / max) * ih;
    const y = padT + ih - bh;
    g += `<rect x="${(cx - barW / 2).toFixed(1)}" y="${y.toFixed(1)}" width="${barW.toFixed(1)}" height="${Math.max(0, bh).toFixed(1)}" rx="3" fill="var(--accent)"><title>${esc(monthLabel(m.month))}: ${fmtEur(m.spend)} · ${m.pieces} pz · ${m.orders} ord.</title></rect>`;
    if (m.spend > 0) g += `<text x="${cx.toFixed(1)}" y="${(y - 5).toFixed(1)}" text-anchor="middle" font-size="10" font-weight="600" fill="var(--txt)">${eurC(m.spend)}</text>`;
    const lbl = monthLabel(m.month);
    g += `<text x="${cx.toFixed(1)}" y="${H - 12}" text-anchor="middle" font-size="10" fill="var(--sub)">${esc(lbl)}</text>`;
  });
  return `<div style="overflow-x:auto"><svg viewBox="0 0 ${W} ${H}" width="${W}" style="max-width:100%;height:auto;display:block" role="img" aria-label="Andamento mensile spesa">${g}</svg></div>`;
}

// ---- ciambella SVG (quota SPESA per fornitore) ----
// data: [{name,spend,pieces}] già ordinata desc. Accorpa la coda oltre 6 voci in "Altri".
function donutChart(rows, total) {
  if (!total) return '';
  const top = rows.slice(0, 6);
  const restSpend = rows.slice(6).reduce((a, r) => a + r.spend, 0);
  const items = restSpend > 0 ? [...top, { name: 'Altri', spend: restSpend }] : top;
  const S = 160, cx = S / 2, cy = S / 2, r = 66, sw = 26;
  const C = 2 * Math.PI * r;
  let off = 0;
  let arcs = '';
  items.forEach((it, i) => {
    const frac = it.spend / total;
    const len = frac * C;
    const pct = Math.round(frac * 100);
    arcs += `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${sliceColor(i)}" stroke-width="${sw}"
      stroke-dasharray="${len.toFixed(2)} ${(C - len).toFixed(2)}" stroke-dashoffset="${(-off).toFixed(2)}"
      transform="rotate(-90 ${cx} ${cy})"><title>${esc(it.name)}: ${fmtEur(it.spend)} (${pct}%)</title></circle>`;
    off += len;
  });
  const legend = items.map((it, i) => {
    const pct = Math.round((it.spend / total) * 100);
    return `<div style="display:flex;align-items:center;gap:7px;font-size:12.5px;margin-bottom:5px">
      ...
      <span class="tnum muted" style="flex-shrink:0"><b style="color:var(--txt)">${pct}%</b> · ${fmtEur(it.spend)}</span>
    </div>`;
  }).join('');
  return `<div style="display:flex;flex-wrap:wrap;gap:18px;align-items:center">
    <svg viewBox="0 0 ${S} ${S}" width="${S}" style="max-width:100%;height:auto;flex-shrink:0" role="img" aria-label="Quota spesa per fornitore">
      ${arcs}
      <text x="${cx}" y="${cy - 2}" text-anchor="middle" font-size="17" font-weight="600" fill="var(--txt)">${eurC(total)}</text>
      <text x="${cx}" y="${cy + 15}" text-anchor="middle" font-size="10" fill="var(--sub)">spesa</text>
    </svg>
...
export function render() {
  ...
  // KPI — spesa in primo piano, pezzi come dato secondario
  h += `<div class="grid k4" style="margin-bottom:12px">
    <div class="card kpi"><div class="lbl">Spesa totale</div><div class="val tnum">${fmtEur(r.totals.spend)}</div></div>
  ...
  h += `<div class="grid k3" style="margin-bottom:16px">
    <div class="card kpi"><div class="lbl">Spesa media / ordine</div><div class="val tnum">${fmtEur(r.avgSpendPerOrder)}</div></div>
    <div class="card kpi"><div class="lbl">Fornitore per spesa</div><div class="val" style="font-size:15px;font-weight:600" title="${esc(r.topSupplier)}">${esc(r.topSupplier)}</div></div>
    <div class="card kpi"><div class="lbl">Prodotto per spesa</div><div class="val" style="font-size:15px;font-weight:600" title="${esc(r.topProduct)}">${esc(r.topProduct)}</div></div>
  </div>`;

  if (r.partialPrices) {
    h += `<div class="card" style="border-color:var(--accent);margin-bottom:16px;padding:11px 14px">
      <span style="font-size:12.5px">⚠️ <b>Prezzi parziali</b> — alcune righe non hanno un prezzo salvato; la spesa usa il prezzo attuale del prodotto (o 0) ed è approssimata.</span></div>`;
  }

  // Andamento mensile (spesa) — grafico a barre SVG
  if (r.byMonth.length > 1) {
    h += `<div class="section-title">Andamento mensile (spesa)</div><div class="card">${barChart(r.byMonth)}</div>`;
  }

  // Quota per fornitore (spesa) — ciambella + legenda
  if (r.bySupplier.length) {
    h += `<div class="section-title" style="margin-top:16px">Quota spesa per fornitore</div><div class="card">${donutChart(r.bySupplier, r.totals.spend)}</div>`;
  }

  // Ripartizione per categoria (spesa)
  if (r.byCategory.length) {
    const maxC = r.byCategory[0]?.spend || 0;
    h += `<div class="section-title" style="margin-top:16px">Spesa per categoria</div><div class="card">${r.byCategory.map(c => bar(c.name, c.spend, maxC, fmtEur(c.spend), `${c.pieces} pz · ${c.righe} righe`)).join('')}</div>`;
  }
  ...
  h += `<div class="section-title" style="margin-top:16px">Spesa per fornitore</div><div class="card">${r.bySupplier.map(s => bar(s.name, s.spend, maxS, fmtEur(s.spend), `${s.pieces} pz · ${s.ordini} ord.`)).join('')}</div>`;
...
function exportReport() {
  ...
  const body = `
    ...
    <div class="meta">${esc(filterSummary())} · ${r.totals.orders} ordini · spesa ${fmtEur(r.totals.spend)} · ${r.totals.pieces} pezzi · ${r.totals.suppliers} fornitori · spesa media/ordine ${fmtEur(r.avgSpendPerOrder)} · generato il ${new Date().toLocaleDateString('it-IT')}</div>
    ${r.partialPrices ? `<div class="notebox">⚠️ Prezzi parziali: alcune righe non hanno un prezzo salvato; la spesa usa il prezzo attuale del prodotto (o 0) ed è approssimata.</div>` : ''}
    ${r.byMonth.length > 1 ? `<h2>Andamento mensile</h2>
    <table><thead><tr><th>Mese</th><th style="text-align:right">Spesa</th><th style="text-align:right">Ordini</th><th style="text-align:right">Pezzi</th></tr></thead>
      <tbody>${rows(r.byMonth, [{ v: o => monthLabel(o.month) }, { v: o => fmtEur(o.spend), r: 1 }, { v: o => o.orders, r: 1 }, { v: o => o.pieces, r: 1 }])}</tbody></table>` : ''}
    <h2>Spesa per fornitore</h2>
    <table><thead><tr><th>Fornitore</th><th style="text-align:right">Spesa</th><th style="text-align:right">%</th><th style="text-align:right">Pezzi</th><th style="text-align:right">Ordini</th></tr></thead>
      <tbody>${rows(r.bySupplier, [{ v: o => o.name }, { v: o => fmtEur(o.spend), r: 1 }, { v: o => (r.totals.spend ? Math.round(o.spend / r.totals.spend * 100) : 0) + '%', r: 1 }, { v: o => o.pieces, r: 1 }, { v: o => o.ordini, r: 1 }])}</tbody></table>
    <h2>Spesa per categoria</h2>
    <table><thead><tr><th>Categoria</th><th style="text-align:right">Spesa</th><th style="text-align:right">Pezzi</th><th style="text-align:right">Righe</th></tr></thead>
